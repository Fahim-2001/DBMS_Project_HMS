...
const CurrentUsersAppt = () => {
  const { singleUser } = useContext(UserDataContext);
  const [appointments, setAppointments] = useState()

  useEffect(() => {
    const getAppts = async () => {
      await fetch(`http://localhost:3000/api/appointments/${singleUser?.email}`, {
        cache: "no-store",
      })
        .then((res) => res.json())
    ...
    <tbody>
      {appointments?.map((appt) => (
        <tr key={appt?.appt_id}>
          ...
          <td>
            {appt?.appt_status === "Unchecked" ? <p>Not Uploaded</p> : 
              <Link
              href={`/profile/appointments/${appt?.appt_id}`}
...
export default CurrentUsersAppt;
